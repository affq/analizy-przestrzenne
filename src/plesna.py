...
drogi_utwardzone = arcpy.analysis.Select(skdr, 'drogi_utwardzone', query)
...
# kryterium 5
arcpy.ddd.Slope(nmt, f"{geobaza}\\slope", "PERCENT_RISE", 1)
slope_fuzzy = arcpy.sa.FuzzyMembership(f"{geobaza}\\slope", fuzzy_function="LINEAR 10 8")
slope_fuzzy.save(f'{geobaza}\\kryterium_5')

#kryterium 6
aspect = arcpy.ddd.Aspect(nmt, f"{geobaza}\\aspect")
aspect_fuzzy = arcpy.sa.FuzzyMembership(f"{geobaza}\\aspect", fuzzy_function="LINEAR 90 113")
aspect_fuzzy_1 = arcpy.sa.FuzzyMembership(f"{geobaza}\\aspect", fuzzy_function="LINEAR 270 248")
